...
def dictionary_creation(event):
    ...
    with open(tf, 'r') as inputfile:
        ...
        for row in reader:
            csv_data.append(row) # To be used for XML formatting
...
def dict_to_xml(event):
    with open("data_file_xml.xml", "w") as write_file: #creating a file with xml extension that can be written
        ...
        for row in csv_data: #csv_data is the data from the csv file in the form of a list
            xml_content += "\t<row>\n"
            xml_content += "\t\t<Order ID>" + row[0] + "</Order ID>\n"
            xml_content += "\t\t<Customer Name>" + row[1] + "</Customer Name>\n"
            xml_content += "\t</row>\n"
        xml_content += "</data>\n"
